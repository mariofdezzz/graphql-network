...
export function useRequestTimings(request: Ref<GraphQLRequest>, timelineStartAt: Ref<Date>) {
  const queueing = computed(() => request.value.timings._blocked_queueing ?? 0)
  const stalled = computed(() => (request.value.timings.blocked ?? 0) - queueing.value)
  ...
  return { queueing, stalled, sent, wait, download, total, requestStartedAt, formatTime }
}
